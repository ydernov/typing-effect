...
describe("resume method tests", () => {
  ...
  describe("call resume on initialized instance", () => {
  ...
  describe("call resume on ready instance", () => {
    test("instance was created with callback and strings", () => {
      ...
      expect(() => te.resume()).toThrow(
        'The method "resume" could not be called on the instance of TypingEffect - the current state "ready" does not match the allowed state "running".'
      );
  ...
  describe("call resume on running instance", () => {
    test("normal scenario - call resume after pause while typing, should pause and idle calling callback with cursorBlinkRate intervals, then resume", () => {
      const cb = vi.fn();
    ...
    test("call resume without calling pause first, should not affect runningState", () => {
      const cb = vi.fn();
      const te = new TypingEffect(["first string", "second string"], cb, {
        delayBeforeTyping: 0,
        delayBeforeUntyping: 0,
  ...
  test("calling after dispose", () => {
    ...
    expect(() => te.resume()).toThrow(
      'The method "resume" could not be called - the instance of TypingEffect was disposed. Create a new one.'
    );
  });
});
